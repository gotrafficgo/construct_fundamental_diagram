...
function Edie_FD(root, TrajectoryData, DataInformation, WaveSpeed, figID, style, whichPart)
    ...
    %% save vQK
    headers = {'givenSpeed_km_h', 'Flow_veh_h', 'Density_veh_km'};
    vQK_table = array2table(vQK, 'VariableNames', headers);
    
    output_path = fullfile(root, 'vQK', DataInformation.dataset, DataInformation.site, DataInformation.lane);   
    if ~exist(output_path, 'dir')
        mkdir(output_path);
    end
           
    full_path = fullfile(output_path, [DataInformation.data '.csv']);
    
    writetable(vQK_table, full_path, 'Encoding', 'UTF-8', 'Delimiter', ',', 'LineEnding', '\n', 'WriteVariableNames', true);
